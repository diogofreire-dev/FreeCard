...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO cards (user_id, name, limit_amount, balance, active, color, bank)
                VALUES (:uid, :name, :limit, :balance, 1, :color, :bank)
            ");
            $stmt->execute([
                ':uid' => $uid,
                ':name' => $name,
          ...
          <div class="card">
            <div class="card-body p-4">
              ...
              <div class="card-preview" id="cardPreview" style="background: <?=$cardColors['purple']['gradient']?>;">
                <div>
                  <div class="mb-3">
                    <i class="bi bi-credit-card" id="cardIcon" style="font-size: 32px;"></i>
                    <?php foreach($bankLogos as $bankKey => $logoUrl): ?>
                      <img src="<?=$logoUrl?>" alt="<?=$bankKey?>" class="bank-logo-card" id="logo-<?=$bankKey?>" data-bank="<?=$bankKey?>">
                    <?php endforeach; ?>
                  </div>
